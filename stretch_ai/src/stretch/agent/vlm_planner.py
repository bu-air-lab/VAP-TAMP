...
class VLMPlanner:
    def __init__(self, agent: RobotAgent, api_key: Optional[str] = None) -> None:
        ...
        prompt_file = os.path.join(current_dir, "..", "llms", "prompts", "obj_centric_vlm.txt")
        ...
        if vlm_option == "gemini":
            ...
            if not api_key:
                api_key = input("You are using Gemini for planning, please type in your Gemini API key: ")
    ...
    def plan(
        ...
        show_prompts=False,
        show_plan=False,
        plan_file: str = "maps/vlm_plan.txt",
        query=None,
        ...
        if show_plan:
            ...
            if output == "explore":
                ...
                actions = []  # Return empty actions for explore
            elif output == "gpt API error":
                print(">>>>>> there is something wrong with the planner api >>>>>>>>>")
                actions = []  # Return empty actions for API error
            else:
                ...
                plt.savefig("maps/plan.png")

        if self.parameters.get("save_vlm_plan", True):
            # Create directory if it doesn't exist
            import os
            os.makedirs(os.path.dirname(plan_file) if os.path.dirname(plan_file) else ".", exist_ok=True)
