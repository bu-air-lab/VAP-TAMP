...
class HomeRobotZmqClient(AbstractRobotClient):
    ...
    def __init__(
        ...
        if enable_rerun_server:
            ...
            self._rerun = RerunVisualizer(output_path=output_path, spawn_gui=False)
        else:
            self._rerun = None
    ...
    def get_observation(self, max_iter: int = 5):
        ...
        with self._obs_lock:
            ...
            # CRITICAL FIX: Recompute XYZ from depth if not present
            if 'xyz' not in self._obs or self._obs['xyz'] is None:
                camera = Camera.from_K(
                    self._obs["camera_K"], self._obs["rgb"].shape[0], self._obs["rgb"].shape[1]
                )
                self._obs['xyz'] = camera.depth_to_xyz(self._obs["depth"])
            ...
            observation = Observations(
                ...
                xyz=self._obs["xyz"],
                lidar_points=self._obs.get("lidar_points"),
                lidar_timestamp=self._obs.get("lidar_timestamp"),
            )
    ...
    def wait_for_waypoint(
        ...
        verbose: bool = False,
        timeout: float = 120.0,  # Increased from 20s to 120s for long distance navigation
    ) -> bool:
        ...
        last_pos = None
        stationary_count = 0
        stationary_threshold = 3  # Need 3 consecutive stationary checks to consider "reached"
        ...
        while not self._finish:
            ...
            # Check if robot has stopped moving (additional success condition)
            if last_pos is not None:
                movement = np.linalg.norm(curr[:2] - last_pos[:2])
                if movement < 0.05:  # Robot hasn't moved more than 5cm
                    stationary_count += 1
                    if stationary_count >= stationary_threshold and pos_err < pos_err_threshold * 2:
                        # Robot is stationary and reasonably close to goal
                        logger.info(f"[WAIT FOR WAYPOINT] Robot stationary near goal (distance: {pos_err:.2f}m)")
                        return True
                else:
                    stationary_count = 0
            last_pos = curr.copy()
    ...
    def get_footprint(self):
        ...
        # Get footprint from parameters if available, otherwise use default Segway dimensions
        if hasattr(self, 'parameters'):
            try:
                fp = self.parameters.get('robot_footprint', None)
                if fp is not None:
                    return Footprint(
                        width=fp.get('width', 0.25),
                        length=fp.get('length', 0.3),
                        width_offset=fp.get('width_offset', 0.0),
                        length_offset=fp.get('length_offset', 0.0)
                    )
            except (KeyError, AttributeError):
                pass

        # Default Segway footprint (approximate)
        return Footprint(width=0.25, length=0.3, width_offset=0.0, length_offset=0.0)

    def navigate_to_goal(self, x: float, y: float, theta: float = 0.0) -> bool:
        """Send a navigation goal to move_base through ZMQ server.

        This sends a goal to the Segway robot's move_base navigation stack.
        The goal is sent through the ZMQ bridge which publishes it to /move_base_simple/goal.

        Args:
            x (float): Target x position in map frame
            y (float): Target y position in map frame
            theta (float): Target orientation in radians (default: 0.0)

        Returns:
            bool: True if goal was successfully sent
        """
        try:
            logger.info(f"Sending navigation goal: x={x:.2f}, y={y:.2f}, theta={theta:.2f}")
            next_action = {"navigation_goal": [float(x), float(y), float(theta)]}
            logger.info(f"Action dict created: {next_action}")
            logger.info(f"Calling send_action with reliable=True...")
            result = self.send_action(next_action, reliable=True)
            logger.info(f"send_action returned: {result}")
            return True
    ...
    def navigate_to_semantic_instance(self, instance, approach_distance: float = 0.5) -> bool:
        """Navigate to a semantic instance from the 3D voxel map.

        Args:
            instance: Semantic instance with 3D position
            approach_distance (float): Distance to maintain from target (meters)

        Returns:
            bool: True if navigation goal was successfully sent
        """
        try:
            # Get 3D center of the instance
            target_center = instance.get_center()
            if target_center is None:
                logger.error("Could not get center of target instance")
                return False

            # Project to 2D (just use x, y from 3D position)
            goal_x = float(target_center[0])
            goal_y = float(target_center[1])

            # Calculate approach position
            current_pose = self.get_base_pose()
            if current_pose is None:
                logger.error("Could not get current robot pose")
                return False

            start_x, start_y = current_pose[0], current_pose[1]
            approach_vector = np.array([start_x - goal_x, start_y - goal_y])
            approach_distance_current = np.linalg.norm(approach_vector)

            if approach_distance_current > 0.01:  # Avoid division by zero
                approach_unit = approach_vector / approach_distance_current
                approach_pos = np.array([goal_x, goal_y]) + approach_unit * approach_distance
                goal_x, goal_y = approach_pos[0], approach_pos[1]

            # Calculate goal orientation (face the object)
            goal_theta = np.arctan2(target_center[1] - goal_y, target_center[0] - goal_x)

            logger.info(f"Navigating to instance at ({goal_x:.2f}, {goal_y:.2f}, {goal_theta:.2f})")
            return self.navigate_to_goal(goal_x, goal_y, goal_theta)

        except Exception as e:
            logger.error(f"Failed to navigate to semantic instance: {e}")
            return False
    ...
    def blocking_spin(self, verbose: bool = False, visualize: bool = False):
        ...
        while not self._finish:
            try:
                ...
                self._seq_id += 1
                # CRITICAL FIX: Decompress received data
                output["rgb"] = compression.from_jpg(output["rgb"])
                output["depth"] = compression.from_jp2(output["depth"]) / 1000.0

                if camera is None and 'camera_K' in output and output['camera_K'] is not None:
                    camera = Camera.from_K(
                        output["camera_K"], output["rgb"].shape[1], output["rgb"].shape[0]
                    )

                if camera:
                    output["xyz"] = camera.depth_to_xyz(output["depth"])
                else:
                    output["xyz"] = None # Cannot compute without K

                if visualize and not shown_point_cloud and output["xyz"] is not None:
                ...
                if verbose and steps % 10 == 0:
                    print(f"[OBS] time taken = {dt:.4f} avg = {sum_time/steps:.4f}")
                t0 = timeit.default_timer()
            except Exception as e:
                logger.error(f"Error in blocking_spin: {e}")
                time.sleep(0.1)
    ...
    def blocking_spin_servo(self, verbose: bool = False):
        ...
        while not self._finish:
            try:
                ...
                logger.error(f"Error in blocking_spin_servo: {e}")
                time.sleep(0.1)
    ...
    def blocking_spin_state(self, verbose: bool = False):
        ...
        while not self._finish:
            try:
                ...
                logger.error(f"Error in blocking_spin_state: {e}")
                time.sleep(0.1)
    ...
    def stop(self):
        ...
        if hasattr(self, '_thread') and self._thread is not None:
            self._thread.join()
        if hasattr(self, '_state_thread') and self._state_thread is not None:
            self._state_thread.join()
        if hasattr(self, '_servo_thread') and self._servo_thread is not None:
            self._servo_thread.join()
        if hasattr(self, '_rerun_thread') and self._rerun_thread is not None:
            self._rerun_thread.join()

        # Close the sockets only if they were created
        if hasattr(self, 'recv_socket'):
            self.recv_socket.close()
        if hasattr(self, 'recv_state_socket'):
            self.recv_state_socket.close()
        if hasattr(self, 'recv_servo_socket'):
