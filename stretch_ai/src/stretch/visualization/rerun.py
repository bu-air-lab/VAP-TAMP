...
class RerunVisualizer:
    ...
    def __init__(
        ...
        # Initialize and connect to viewer
        logger.info(f"Rerun initialization: spawn_gui={spawn_gui}, open_browser={open_browser}")
        ...
        # CRITICAL: Store spawn state for later waiting
        self._spawn_gui = spawn_gui
        ...
        # Ensure viewer is ready before logging any data
        self._ensure_viewer_ready()
    ...
    def _ensure_viewer_ready(self):
        """Ensure viewer is ready before logging data (prevents BufferedSink warning)."""
        if not self._viewer_ready and self._spawn_gui:
            logger.info("Waiting for Rerun viewer to fully initialize...")
            time.sleep(2.5)  # Wait for spawned viewer to connect
            self._viewer_ready = True
            logger.info("✅ Rerun viewer ready")
    ...
    def step(self, obs, servo):
        """Log all the data"""
        # Ensure viewer is ready before first log (prevents BufferedSink warning)
        self._ensure_viewer_ready()
