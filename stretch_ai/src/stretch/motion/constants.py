...
class HelloStretchIdx:
    ...
    # Add name to index mapping for joint states
    # Original Stretch robot mapping
    stretch_name_to_idx = {
        "base_x": BASE_X,
        "base_y": BASE_Y, 
    ...
    # Generic robot mapping (for UR5 + mobile base)
    # Map to available joint indices with reasonable defaults
    generic_name_to_idx = {
        "base_x": 0,  # Use shoulder_pan_joint as base motion proxy
        "base_y": 1, 
        "base_theta": 2,
        "lift": 1,  # shoulder_lift_joint
        "arm": 2,   # elbow_joint 
        "gripper": 6,  # finger_joint
        "wrist_roll": 3,   # wrist_1_joint
        "wrist_pitch": 4,  # wrist_2_joint  
        "wrist_yaw": 5,    # wrist_3_joint
        "head_pan": 0,     # No head joints, use shoulder_pan
        "head_tilt": 1     # No head joints, use shoulder_lift
    }
    
    # Use generic mapping as default (will work with current robot)
    name_to_idx = generic_name_to_idx
