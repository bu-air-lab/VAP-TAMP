...
class SparseVoxelMapNavigationSpace(XYT):
    ...
    def __init__(
        ...
        custom_footprint: Optional[dict] = None,
    ):
        self.robot = robot
        self.step_size = step_size
        self.rotation_step_size = rotation_step_size
        self.voxel_map = voxel_map
        self.custom_footprint = custom_footprint
    ...
    def create_collision_masks(self, orientation_resolution: int, show_all: bool = False):
        ...
        """
        # Handle case where robot model is None (e.g., for ROS2 robots)
        if self.robot is not None:
            self._footprint = self.robot.get_footprint()
        else:
            # Create default footprint for navigation
            from stretch.motion import Footprint
            # Check if custom footprint is specified
            if self.custom_footprint:
                fp_params = self.custom_footprint
                self._footprint = Footprint(
                    width=fp_params["width"],
                    length=fp_params["length"],
                    width_offset=fp_params["width_offset"],
                    length_offset=fp_params["length_offset"]
                )
                print(f"🤖 Using custom Segway footprint: {fp_params['width']}m x {fp_params['length']}m")
            else:
                self._footprint = Footprint(width=0.4, length=0.4, width_offset=0.0, length_offset=0.0)
                print(f"🤖 Using default robot footprint: 0.4m x 0.4m")
    ...
    def _get_open3d_geometries(
        ...
        **backend_kwargs,
        ...
        if points is not None:
            ...
            print("No point cloud found in this frame, skipping...")
            return None  # Or continue, or handle gracefully
        ...
        # Visualize traversible area and frontier from the motion planner - DISABLED to remove red/green colors
    ...
    def show(
        ...
        # Show the geometries of where we have explored with better camera positioning
        ...
        # Get the view control to set camera position
        ctr = vis.get_view_control()
        
        # Set camera to side view instead of looking down
        # This positions the camera to look from the side at the map
        ctr.set_front([1, 0, 0])  # Look along positive X axis (side view)
        ctr.set_lookat([0, 0, 0])  # Look at origin
        ctr.set_up([0, 0, 1])      # Z is up direction
