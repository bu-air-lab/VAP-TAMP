...
class SparseVoxelMap(object):
    ...
    def __init__(
        ...
        self._open3d_vis = None  # Open3D Visualizer
        self._open3d_pcd = None  # PointCloud geometry
        self._open3d_added = False  # Flag to track first-time geometry addition
    ...
    def _show_open3d(self, instances=False, refresh_rate=0.05, **kwargs):
        """Persistent Open3D viewer that updates point cloud in real-time."""

        # Convert voxel map to Open3D geometry
        xyz, colors = self._get_xyz_colors(instances)  # you must implement this
        points = open3d.utility.Vector3dVector(xyz)
        color_vec = open3d.utility.Vector3dVector(colors)

        if self._open3d_vis is None:
            # Initialize visualizer and point cloud once
            self._open3d_vis = open3d.visualization.Visualizer()
            self._open3d_vis.create_window(window_name="Sparse Voxel Map")
            self._open3d_pcd = open3d.geometry.PointCloud()
            self._open3d_pcd.points = points
            self._open3d_pcd.colors = color_vec
            self._open3d_vis.add_geometry(self._open3d_pcd)
            self._open3d_added = True
        else:
            # Update point cloud data
            self._open3d_pcd.points = points
            self._open3d_pcd.colors = color_vec
            self._open3d_vis.update_geometry(self._open3d_pcd)

        # Render updated scene
        self._open3d_vis.poll_events()
        self._open3d_vis.update_renderer()
    ...
    def read_from_pickle(
        ...
        # Only setup visualization if perception is provided
        fig, axes = None, None
        im1, im2 = None, None
        ...
        # Only cleanup visualization if it was set up
        if perception is not None:
            plt.ioff()
            plt.show()
    ...
    def get_2d_map(self, force_update=False, debug: bool = False) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # CRITICAL FIX: Correct 3D coordinate system transformation
        # Fix the coordinate system inversion at the source (3D → grid transformation)

        # Apply coordinate transformation with Y-axis correction
        xyz_grid = (xyz / self.grid_resolution) + self.grid_origin

        # Fix Y-axis orientation: flip Y coordinate to match expected map orientation
        # This ensures the 3D world coordinates properly map to the 2D grid
        xyz_grid[:, 1] = self.grid_size[1] - xyz_grid[:, 1]  # Flip Y-axis in grid space
        ...
        # CRITICAL FIX: Correct obstacle detection logic
        # Floor areas have HIGH density (many depth points), walls have LOWER density
        # So we need to detect obstacles as areas with intermediate density (not floor, not empty)

        # Get floor-level points (should be high density)
        floor_voxels = voxels[:, :, :min_height+2]  # Floor and slightly above
        floor_density = torch.sum(floor_voxels, dim=-1)

        # Get obstacle-level points (furniture, walls)
        obstacle_voxels = voxels[:, :, min_height:]
        obstacle_density = torch.sum(obstacle_voxels, dim=-1)

        # CORRECTED LOGIC: Obstacles are areas with:
        # 1. Some points at obstacle height (not empty space)
        # 2. But not dominated by floor points (not floor)
        obstacles_soft = obstacle_density - floor_density

        # Areas are obstacles if they have obstacle-height points but aren't primarily floor
        obstacles = (obstacle_density > self.obs_min_density) & (obstacles_soft > 0)
        ...
        # CRITICAL FIX: Convert unknown cells to free space where no obstacles detected
        # Instead of only marking areas with point data as explored, assume all non-obstacle areas are navigable

        # First, get the original explored areas (areas with actual point cloud data)
        explored_soft = torch.sum(voxels, dim=-1)

        # Add floor detection for navigation
        if self.use_negative_obstacles:
            # Floor points are already captured in voxels via negative obstacles
            floor_voxels = voxels[:, :, :min_height+1]  # Include floor level
            floor_explored = torch.sum(floor_voxels, dim=-1)
            explored_soft += floor_explored

        # Expand explored area around known points
        if explored_soft.max() > 0:
            # Dilate the explored area slightly to include nearby floor
            explored_dilated = binary_dilation(
                (explored_soft > 0).float().unsqueeze(0).unsqueeze(0),
                torch.ones((1, 1, 3, 3), device=explored_soft.device)
            )[0, 0]
            explored_soft += explored_dilated.float()

        # Add explored radius around the robot
        explored_soft += self._visited

        # MAJOR FIX: Assume all areas are explored unless we have specific obstacles
        # This converts "unknown" cells to "free" cells for navigation
        explored = torch.ones_like(obstacles, dtype=torch.bool, device=obstacles.device)

        # Mark areas as unexplored only if they are:
        # 1. Far from any observed points AND
        # 2. Far from robot visited areas
        if explored_soft.max() > 0:
            # Create a mask of areas that are far from any observations
            distance_from_observations = torch.ones_like(explored_soft, dtype=torch.float)

            # Areas close to observations or robot path are considered explored
            close_to_observations = explored_soft > 0
            if close_to_observations.sum() > 0:
                # Dilate the observed areas to mark nearby regions as explored too
                explored_area_dilated = binary_dilation(
                    close_to_observations.float().unsqueeze(0).unsqueeze(0),
                    torch.ones((1, 1, 5, 5), device=explored_soft.device)  # Larger dilation for more free space
                )[0, 0].bool()

                # Only mark areas as unexplored if they are very far from any observations
                # and not near the robot's path
                unexplored_mask = ~explored_area_dilated
                explored = explored & ~unexplored_mask
        ...
        # Note: 2D map orientation will be fixed by correcting 3D coordinate system
    ...
    def plan_to_grid_coords(self, plan_result: PlanResult) -> Optional[List[torch.Tensor]]:
        ...
        if not plan_result.success:
            ...
            for node in plan_result.trajectory:
                # CRITICAL FIX: Ensure node.state is a numpy array before slicing
                state = node.state
                if isinstance(state, list):
                    state = np.array(state)
                elif hasattr(state, 'cpu'):  # Handle torch tensors
                    state = state.cpu().numpy()

                traj.append(self.grid.xy_to_grid_coords(state[:2]))
            return traj
