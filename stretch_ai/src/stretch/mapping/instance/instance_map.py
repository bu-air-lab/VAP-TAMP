...
class InstanceMemory:
    ...
    def process_instances_for_env(
        ...
        for instance_id in instance_ids:
            ...
            added = False
            # SIMPLIFIED CHECK: Only require basic point count, ALL segmented objects get bounding boxes!
            if n_points > 1 and n_mask > 0:
                bounds = get_bounds(point_cloud_instance)
                volume = float(box3d_volume_from_bounds(bounds).squeeze())
                
                # Always create bounding box for every segmented object
                if verbose:
                    height = (bounds[2][0] + bounds[2][1]) / 2.0 if bounds.shape == (3, 2) else 0.0
                    min_thickness = min(
                        bounds[0][1] - bounds[0][0],
                        bounds[1][1] - bounds[1][0], 
                        bounds[2][1] - bounds[2][0],
                    ) if bounds.shape == (3, 2) else 0.0
                    logger.info(
                        f"✅ CREATING BOUNDING BOX: Vol={volume:.2e}m³, H={height:.3f}m, T={min_thickness:.3f}m, Points={n_points}, Mask={n_mask}"
                    )
                
                # get instance view - CREATE BOUNDING BOX FOR EVERY SEGMENTED OBJECT
