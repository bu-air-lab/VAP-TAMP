...
class TeleopMapper:
    ...
    def setup(self) -> bool:
        ...
        # Tweak mapping parameters for cleaner maps
        params["voxel_size"] = 0.02
        params["obs_min_height"] = 0.05
        params["obs_max_height"] = 2.5
        params["obs_min_density"] = 5
        ...
        # 6. Create Robot Agent with nvblox integration
        print("Creating robot agent with nvblox integration...")
        self.agent = RobotAgent(
            ...
            teleop_only=True,
            use_nvblox=True,  # Enable nvblox integration
            nvblox_topic="/nvblox_node/static_esdf_pointcloud"  # nvblox topic
        )
        if self.agent.use_nvblox:
            print("✓ Robot agent created with nvblox fusion enabled.")
        else:
            print("✓ Robot agent created (nvblox unavailable, using standard mapping).")
